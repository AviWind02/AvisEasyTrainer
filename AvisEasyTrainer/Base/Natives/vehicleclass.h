...
namespace gamebase {
    namespace natives {
        namespace vehicle {
            ...
            inline bool InsertVehicleIntoVehicleList(const RED4ext::TweakDBID& id)
            {
                using namespace RED4ext;
                ...
                if (!tdb)
                {
                    loghandler::sdk->logger->Error(loghandler::handle, "[InsertVehicleIntoVehicleList] TweakDB not available.");
                    return false;
                }

                TweakDBID vehicleListID("Vehicle.vehicle_list.list");

                // 1. Get existing vehicle list
                DynArray<TweakDBID> vehicleList;
                if (!tdb->TryGetValue(vehicleListID, vehicleList))
                {
                    loghandler::sdk->logger->Error(loghandler::handle, "[InsertVehicleIntoVehicleList] Failed to read Vehicle.vehicle_list.list");
                    return false;
                }

                // 2. Skip if already present
                if (std::find(vehicleList.begin(), vehicleList.end(), id) != vehicleList.end())
                {
                    loghandler::sdk->logger->InfoF(loghandler::handle, "[InsertVehicleIntoVehicleList] Vehicle already exists: %s", id);
                    return true;
                }

                // 3. Add new ID
                vehicleList.PushBack(id);

                // 4. Prepare CStackType for new flat value
                CStackType stack;
                stack.type = CRTTISystem::Get()->GetType("array:TweakDBID");
                stack.value = &vehicleList;


                // 5. Allocate and write the new flat
                int32_t newOffset = tdb->CreateFlatValue(stack);
                if (newOffset == -1)
                {
                    loghandler::sdk->logger->Error(loghandler::handle, "[InsertVehicleIntoVehicleList] Failed to create new flat value");
                    return false;
                }

                // 6. Update offset on the TweakDBID
                vehicleListID.SetTDBOffset(newOffset);



                if (!tdb->GetRecord(vehicleListID))
                {
                    loghandler::sdk->logger->Error(loghandler::handle,
                        "[InsertVehicleIntoVehicleList] Vehicle list record 'Vehicle.vehicle_list.list' does not exist. Skipping vehicle add.");
                    return false;
                }

                bool addedFlat = tdb->AddFlat(vehicleListID);
                if (!addedFlat)
                {
                    loghandler::sdk->logger->Warn(loghandler::handle, "[InsertVehicleIntoVehicleList] AddFlat failed - may already exist");
                }
                else
                {
                    if (!tdb->UpdateRecord(vehicleListID))
                    {
                        loghandler::sdk->logger->Error(loghandler::handle, "[InsertVehicleIntoVehicleList] UpdateRecord failed");
                        return false;

                    }
                }

                loghandler::sdk->logger->InfoF(loghandler::handle, "[InsertVehicleIntoVehicleList] Vehicle %s added to list successfully", id);
                return true;
            }
